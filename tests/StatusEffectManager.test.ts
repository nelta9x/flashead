...
describe('StatusEffectManager', () => {
  ...
  describe('applyEffect / getEffects', () => {
    it('엔티티에 효과를 부착하고 조회할 수 있어야 함', () => {
      ...
      manager.applyEffect('entity-1', effect);

      const effects = manager.getEffects('entity-1');
      expect(effects).toHaveLength(1);
      expect(effects[0].id).toBe('effect-1');
    });

    it('같은 엔티티에 여러 효과를 부착할 수 있어야 함', () => {
      manager.applyEffect('entity-1', createEffect({ id: 'e1', type: 'poison' }));
      manager.applyEffect('entity-1', createEffect({ id: 'e2', type: 'burn' }));

      expect(manager.getEffects('entity-1')).toHaveLength(2);
    });
  ...
  describe('removeEffect', () => {
    it('특정 효과를 제거할 수 있어야 함', () => {
      manager.applyEffect('entity-1', createEffect({ id: 'e1' }));
      manager.applyEffect('entity-1', createEffect({ id: 'e2' }));

      manager.removeEffect('entity-1', 'e1');
    ...
    it('마지막 효과 제거 시 엔티티 항목이 정리되어야 함', () => {
      ...
      expect(manager.getEffects('entity-1')).toHaveLength(0);
    });

    it('존재하지 않는 효과 제거는 무해해야 함', () => {
      manager.applyEffect('entity-1', createEffect({ id: 'e1' }));
      manager.removeEffect('entity-1', 'nonexistent');

      expect(manager.getEffects('entity-1')).toHaveLength(1);
    });
  });
  ...
  describe('tick', () => {
    ...
    it('매 틱마다 onTick이 호출되어야 함', () => {
      ...
      expect(onTick).toHaveBeenCalledWith('entity-1', 100, effect.data);
    });
    ...
    it('만료 시 onExpire가 호출되어야 함', () => {
      ...
      expect(onExpire).toHaveBeenCalledWith('entity-1', effect.data);
    });
    ...
    it('여러 엔티티의 효과가 독립적으로 틱되어야 함', () => {
      const onTick1 = vi.fn();
      const onTick2 = vi.fn();
      manager.applyEffect('entity-1', createEffect({ id: 'e1', duration: 1000, onTick: onTick1 }));
      manager.applyEffect('entity-2', createEffect({ id: 'e2', duration: 1000, onTick: onTick2 }));

      manager.tick(100);

      expect(onTick1).toHaveBeenCalledWith('entity-1', 100, {});
      expect(onTick2).toHaveBeenCalledWith('entity-2', 100, {});
    });
  });
  ...
  describe('clearEntity', () => {
    ...
    it('다른 엔티티의 효과에는 영향이 없어야 함', () => {
      manager.applyEffect('entity-1', createEffect({ id: 'e1' }));
      manager.applyEffect('entity-2', createEffect({ id: 'e2' }));

      manager.clearEntity('entity-1');

      expect(manager.getEffects('entity-2')).toHaveLength(1);
    });
  });

  describe('hasEffect / getEffectsByType', () => {
    it('특정 타입 효과 존재 여부를 확인할 수 있어야 함', () => {
      manager.applyEffect('entity-1', createEffect({ type: 'poison' }));

      expect(manager.hasEffect('entity-1', 'poison')).toBe(true);
      expect(manager.hasEffect('entity-1', 'burn')).toBe(false);
    });

    it('타입별로 효과를 필터링할 수 있어야 함', () => {
      manager.applyEffect('entity-1', createEffect({ id: 'e1', type: 'poison' }));
      manager.applyEffect('entity-1', createEffect({ id: 'e2', type: 'burn' }));
      manager.applyEffect('entity-1', createEffect({ id: 'e3', type: 'poison' }));

      const poisons = manager.getEffectsByType('entity-1', 'poison');
  ...
  describe('clear', () => {
    it('모든 엔티티의 모든 효과를 제거해야 함', () => {
      ...
      expect(manager.getEffects('entity-1')).toHaveLength(0);
      expect(manager.getEffects('entity-2')).toHaveLength(0);
    });
  });

  describe('freeze/slow 내장 효과', () => {
    it('freeze 효과를 부착하고 hasEffect로 확인할 수 있어야 함', () => {
      manager.applyEffect('entity-1', createEffect({
        id: 'entity-1:freeze',
        type: 'freeze',
        duration: Infinity,
        remaining: Infinity,
      ...
      expect(manager.hasEffect('entity-1', 'freeze')).toBe(true);
      expect(manager.hasEffect('entity-1', 'slow')).toBe(false);
    });
    ...
    it('freeze 효과를 수동 제거할 수 있어야 함', () => {
      ...
      manager.removeEffect('entity-1', 'entity-1:freeze');

      expect(manager.hasEffect('entity-1', 'freeze')).toBe(false);
    });
    ...
    it('slow 효과를 부착하고 factor 데이터를 조회할 수 있어야 함', () => {
      ...
      const slowEffects = manager.getEffectsByType('entity-1', 'slow');
